...
% 10/10/2014, in vivo, brain, patient: J.F. Nielsen
...
nfd = length(flip_d);
TRd = 14.6 * ones(nfd, 1);          % ms
TEd = TE_global * ones(nfd, 1);     % ms
wf = 0;                             % Zero off-resonance
sl = 5;                             % Latter slice chosen due to wrap-around
...
if (~exist('yp_im', 'var'))
    try
        addpath('Brain_10,10,14/');
        load(sprintf('ims_coil_comb_slice%u.mat', sl));
    catch
        dir = pwd; 
        cd('/Volumes/General Storage/Documents/Research /DESS/2014,10,10_brain_32ch');

        yp_im = NaN(nx, ny, nfd);
        ym_im = NaN(nx, ny, nfd);
        for l = 1:nfd
            load(sprintf('dess_jfn_10Oct2014_frame%u', l));
        ...
        % Take transpose to match baseline orientation
        % Flip to image anterior --> posterior
        % This is now coil-combined complex data
        yp_im = flipdim(permute(yp_im, [2 1 3]), 2);
        ym_im = flipdim(permute(ym_im, [2 1 3]), 2); 
    end
end
...
tight_mask = imfill(squeeze(abs(yp_im(:,:,ceil(nfd/2))) >...
    0.1*max(col(abs(yp_im(:,:,ceil(nfd/2)))))), 'holes');
...
% M0s Images 
figure; im('notick', abs(M0s_ml),  [0 16], 'cbar', ' ');
print('-deps','M0s_ml_10,40.eps');
figure; im('notick', abs(M0s_med), [0 16], 'cbar', ' ');
print('-deps','M0s_med_10,40.eps');
figure; im('notick', abs(M0s_reg), [0 16], 'cbar', ' ');
print('-deps','M0s_reg_10,40.eps');

% T1 Images
figure; im('notick', T1_ml,  [0 3000], 'cbar', ' ');
print('-deps','T1_ml_10,40.eps');
figure; im('notick', T1_med, [0 3000], 'cbar', ' ');
print('-deps','T1_med_10,40.eps');
figure; im('notick', T1_reg, [0 3000], 'cbar', ' ');
print('-deps','T1_reg_10,40.eps');

% T2 Images
figure; im('notick', T2_mom, [0 150], 'cbar', ' ');
print('-deps','T2_mom_10,40.eps');
figure; im('notick', T2_ml,  [0 150], 'cbar', ' ');
print('-deps','T2_ml_10,40.eps');
figure; im('notick', T2_med, [0 150], 'cbar', ' ');
print('-deps','T2_med_10,40.eps');
figure; im('notick', T2_reg, [0 150], 'cbar', ' ');
print('-deps','T2_reg_10,40.eps');

% kap Images
figure; im('notick', kap_ml,  [0 2], 'cbar', ' ');
print('-deps','kap_ml_10,40.eps');
figure; im('notick', kap_med, [0 2], 'cbar', ' ');
print('-deps','kap_med_10,40.eps');
figure; im('notick', kap_reg, [0 2], 'cbar', ' ');
print('-deps','kap_reg_10,40.eps');
...
fid = fopen('Brain_10,10,14_summary_10,40', 'a');
...
for r = 1:nROI
    ...
    print('-depsc', sprintf('sig_vs_10,40_ROI%u.eps', r));
end
