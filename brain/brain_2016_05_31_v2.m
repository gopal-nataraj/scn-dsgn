...
%   v3.1    2016-09-21    new image layout
...
addpath('../../model/se');
addpath('../../etc');
addpath('../../../../../contrib/source/web');

% raw data path
% datdir = '/Volumes/a2/gnataraj/Data/2016,05,31_brain_32ch';
datdir = '/Volumes/hd-2tb/Documents/school/michigan/research/data/2016,05,31_brain_32ch';
...
try 
  ...
  addpath([datdir '/matlab']);
  addpath([datdir '/wav']);
  addpath('../../../../../contrib/source/jfnielse/data');
  addpath('../../../../../contrib/source/jfnielse/data/official/recon');
  addpath('../../../../../contrib/source/jfnielse/data/official/img');
  ...
  y.im.sp = ...                         % [n.x n.y C.sp E.sp n.c]
    permute(flipdim(flipdim(y.im.sp(f.cropx,:,n.sl,:,:,:), 1), 2), [2 1 5 6 4 3]); 
  y.im.de = ...                         % [n.x n.y C.de E.de n.c]
    permute(flipdim(flipdim(y.im.de(f.cropx,:,n.sl,:,:,:), 1), 2), [2 1 5 6 4 3]);  
...
if bool.im
  mask.im = false(n.x, n.y);
  crop.x = 35:222;
  crop.y = 11:256;
  mask.im(crop.x, crop.y) = true;
 	n.xcrop = length(crop.x);
  n.ycrop = length(crop.y);
  
  for m = 1:length(opt.im.cmap)
    % anatomical roi
    tmp = masker(ycc.ir, mask.t);
    tmp = embed(tmp, mask.t);
    tmp = tmp(crop.x, crop.y, C.ir);
    figure; im('notick', tmp, [0 8], ' ');...
      colormap(opt.im.cmap{m});...
      hold on;
      for r = 1:length(roi.bound)
        tmp = roi.bound{r};
        for b = 1:length(roi.bound{r})
          tmp2 = tmp{b};
          plot(tmp2(:,1)-crop.x(1)+1, tmp2(:,2)-crop.y(1)+1, roi.color{r}, 'LineWidth', 0.5);
        end
      end
      text(n.xcrop/2, 0, 'A',...
      ...
      hold off;
      if bool.pr
        tmp = sprintf('../../data/Brain_05,31,16/2016-05-31,brain,roi,%s.eps', opt.im.cmap{m});
        print('-depsc', tmp);
      end
    ...
    for i=1:length(tmp)
      tmp2 = tmp(i).ml;
      tmp(i).ml = tmp2(crop.x, crop.y);
    end
    figure; im('notick', 'row', 1, cat(3, tmp(:).ml), rng.t1, 'cbar', ' ');...
      colormap(opt.im.cmap{m});...
      tmp = colorbar; set(tmp, 'YTick', linspace(rng.t1(1), rng.t1(2), 6));...
      hold on;...
      text(col(n.xcrop/2-1:n.xcrop:7*n.xcrop/2), zeros(n.p,1), col(opt.im.prof),...
        'VerticalAlignment', 'bottom',...
        'HorizontalAlignment', 'center',...
        'FontSize', 16,...
        'Color', 'k');
      text(0, n.ycrop/2, opt.im.t1{1},...
        'VerticalAlignment', 'bottom',...
        'HorizontalAlignment', 'center',...
        'FontSize', 16,...
        'Color', 'k',...
      ...
      hold off;
      if bool.pr
        tmp = sprintf('../../data/Brain_05,31,16/2016-05-31,brain,t1-ml,%s.eps', opt.im.cmap{m});
        print('-depsc', tmp);
      end
    ...
    figure; im('notick', 'row', 1, cat(3, tmp(:).ml), rng.t2, 'cbar', ' ');...
      ...
      hold off;
      if bool.pr
        tmp = sprintf('../../data/Brain_05,31,16/2016-05-31,brain,t2-ml,%s.eps', opt.im.cmap{m});
        print('-depsc', tmp);
      end  
    ...
    figure; im('notick', x(p).t1.ml, rng.t1, 'cbar', ' ');...
      if bool.roi
        ...
        for r = 1:length(roi.bound)
          ...
          for b = 1:length(roi.bound{r})
            tmp2 = tmp{b};
            plot(tmp2(:,1), tmp2(:,2), roi.color{r}, 'LineWidth', 0.5);
          end
        end
        hold off;
      end
...
% for predoc fellowship; delete later
% t1 ml
tmp = [x(4).t1 x(1).t1];
for i=1:length(tmp)
  tmp2 = tmp(i).ml;
...
figure; im('notick', 'row', 1, cat(3, tmp(:).ml), rng.t1, 'cbar', ' ');...
  colormap(opt.im.cmap{1});...
  tmp = colorbar; set(tmp, 'YTick', linspace(rng.t1(1), rng.t1(2), 6));...
  hold on;...
  text(col(n.xcrop/2-1:n.xcrop:3*n.xcrop/2), zeros(2,1), {'Gold Standard T1 (41m)';'Proposed T1 (1m40s)'},...
    'VerticalAlignment', 'bottom',...
    'HorizontalAlignment', 'center',...
    'FontSize', 16,...
    'Color', 'k');
