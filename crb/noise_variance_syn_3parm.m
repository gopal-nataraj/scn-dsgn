% Sanity Check Noise Analysis on Synthetic Phantom - Three Parameters
...
%% PART TWO: Three-Parameter VarPro for computing empirical (M0s/T1/T2/kappa) est. variance
% High-resolution dictionary creation
T1 = logspace(log10(500), log10(600), 100);
T2 = logspace(log10(38), log10(42), 100);
kappa = kap_true;                       % Assume known via some other manner
D = NaN(2*length(flip), length(T1), length(T2));
for t1 = 1:length(T1)
    for t2 = 1:length(T2)
        for a = 1:nf
            [D(a, t1, t2), D(nf+a, t1, t2)] = ...
                dess_fun(TR, TE, 1, T1(t1), T2(t2), flip(a)*kappa, wf);
        end
    end
end
...
% Note: norm_crlb_dess_3parm() yields standard deviations for
...
%     M0s_ml_mean, T1_ml_mean, T2_ml_mean, wf, []', flip, [0]', [TR]', TE);
% scan_time = TR * nf;
% 
% % Compensate for noise variance and undo time-compensation for comparison
% M0s_theory_sd = sig_M0s_unnorm * sqrt(sig_var_theo/scan_time);
% T1_theory_sd  = sig_T1_unnorm  * sqrt(sig_var_theo/scan_time);
% T2_theory_sd  = sig_T2_unnorm  * sqrt(sig_var_theo/scan_time);
...
Xm = linspace(min(M0s_ml(roi_msk)), max(M0s_ml(roi_msk)), 100);...
    ...
    [Nm, Bm] = hist(M0s_ml(roi_msk), floor((2*radius+1)^2/50));... 
